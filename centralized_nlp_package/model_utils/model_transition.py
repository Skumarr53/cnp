...
def transition_model(
    ...
    if version is None:
        ...
        if not model_version:
            print("Model version {version} does not exist.")
            return
        _update_stage(model_version, stage)
    print("Model {model_name} transitioned to stage: {stage}")
...
def _register_model( model_name, run ) -> int:
    ...
    print(model_uri)
    print("Registering model from run: {run.info.run_id}")

    # Use mlflow.register_model instead of client.register_model
    model_version = mlflow.register_model(model_uri, model_name)
    
    print("Model registered with version: {model_version.version}")
...
def _fetch_model_version( model_name, version: int, ):
    ...
    try:
        ...
        print("Error fetching model version {version}: {e}")
        return None
...
def _update_stage( model_name, model_version, stage: str):
    ...
    print("Model version {model_version.version} transitioned to {stage}")
